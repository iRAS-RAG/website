...
const TabOverview: React.FC<Props> = ({ batch }) => {
  const [stages, setStages] = useState<PlannedStage[] | undefined>(
    batch.plannedStages,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const feedSum = feeds.reduce(
          (s: number, f: IOperatorFeedingLog) => s + (Number(f.amount) || 0),
          0,
        );
        const mortSum = morts.reduce(
          (s: number, m: IOperatorMortalityLog) =>
            s + (Number(m.quantity) || 0),
          0,
        );
  ...
  const netPercent =
    initialQty > 0 ? ((currentQty ?? 0) / initialQty - 1) * 100 : undefined;
  const estimatedSurvivalPct =
    batch.estimatedHarvestCount != null && initialQty > 0
      ? (batch.estimatedHarvestCount / initialQty) * 100
      : undefined;
  ...
  const actualSurvivalPct =
    isFinished && initialQty > 0 && actualHarvestCount != null
              ...
              {[...stages]
                ...
                .map((s) => {
                  ...
                  const start = parseDate(
                    s.actualStartDate ?? s.estimatedStartDate ?? undefined,
                  );
                  const end = parseDate(
                    s.actualEndDate ?? s.estimatedEndDate ?? undefined,
                  );
                  const isActive =
                    !isFinished &&
                    !!start &&
                    (end ? now >= start && now < end : now >= start);
                        ...
                        <CardContent>
                          ...
                          <Stack direction="column" spacing={1} sx={{ mt: 1 }}>
                            ...
                            >
                              ...
                              >
                                Tần suất/ngày:{" "}
                                <strong>{s.frequencyPerDay ?? "—"} lần</strong>
                              </Typography>
                              {s.amountPer100Fish != null ? (
                                ...
                                >
                                  Số lượng/100:{" "}
                                  <strong>{s.amountPer100Fish}</strong>
                                </Typography>
                              ) : null}
                            </Stack>
                          </Stack>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        >
          <KPICard
            ...
            label="Dung tích bể"
            value={batch.tankVolume ? `${batch.tankVolume} m³` : "-- m³"}
            desc={batch.fishTankName || ""}
          />
          <KPICard
            icon={
            ...
            desc={
              isFinished
                ? actualHarvestCount != null
                  ...
                  : ""
                : batch.estimatedHarvestCount != null
                  ? `Dự kiến số: ${batch.estimatedHarvestCount} con`
                  : ""
            }
          />
          <KPICard
            ...
            value={
              isFinished
                ? actualHarvestCount != null
                  ...
                  : "—"
                : batch.estimatedHarvestCount != null
                  ? `${batch.estimatedHarvestCount} con`
                  : "—"
            }
            desc={
              isFinished
          ...
          <KPICard
            ...
            value={`${totalDead} ${batch.unitOfMeasure}`}
            desc="Số lượng"
          />
        </Box>
      </Box>
