...
function toSensor(item: Record<string, unknown>): Sensor {
  return {
    ...
    sensorTypeId: item.sensorTypeId ? String(item.sensorTypeId) : item.sensor_type_id ? String(item.sensor_type_id) : undefined,
...
export async function createSensor(payload: SensorUpsertPayload): Promise<Sensor> {
...
export async function updateSensor(id: string, payload: SensorUpsertPayload): Promise<Sensor | null> {
  const body: Record<string, unknown> = { ...payload };
...
export async function deleteSensor(id: string): Promise<boolean> {
  await apiFetch(`/hardware/sensors/${id}`, { method: "DELETE" });
  return true;
}
