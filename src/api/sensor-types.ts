...
export async function createSensorType(payload: SensorTypeCreate): Promise<SensorType> {
  const body = { ...payload };
  const created = await apiFetch<Record<string, unknown>>("/hardware/sensor-types", { method: "POST", body });
...
export async function updateSensorType(id: string, payload: Partial<SensorTypeCreate>): Promise<SensorType | null> {
  const body: Record<string, unknown> = {};
  Object.assign(body, payload);
  const updated = await apiFetch<Record<string, unknown>>(`/hardware/sensor-types/${id}`, { method: "PUT", body });
...
export async function deleteSensorType(id: string): Promise<boolean> {
  await apiFetch(`/hardware/sensor-types/${id}`, { method: "DELETE" });
  return true;
}

export default { getSensorTypes, createSensorType, updateSensorType, deleteSensorType };
