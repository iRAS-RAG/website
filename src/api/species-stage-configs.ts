...
function toUi(item: Record<string, unknown>): SpeciesStageConfig {
  return {
    ...
    amountPer100Fish: item.amountPer100Fish ? Number(item.amountPer100Fish) : undefined,
    frequencyPerDay: item.frequencyPerDay ? Number(item.frequencyPerDay) : undefined,
    maxStockingDensity: item.maxStockingDensity ? Number(item.maxStockingDensity) : undefined,
    expectedDurationDays: item.expectedDurationDays ? Number(item.expectedDurationDays) : undefined,
  };
}

export async function getSpeciesStageConfigs(searchTerm?: string): Promise<SpeciesStageConfig[]> {
  const path = searchTerm ? `/species-stage-configs?SearchTerm=${encodeURIComponent(searchTerm)}` : "/species-stage-configs";
...
export async function getSpeciesStageConfig(id: string): Promise<SpeciesStageConfig | null> {
  try {
    const res = await apiFetch<unknown>(`/species-stage-configs/${id}`);
    if (!res) return null;
...
export async function createSpeciesStageConfig(payload: SpeciesStageConfigCreate): Promise<SpeciesStageConfig> {
  const body = { ...payload };
  const created = await apiFetch<Record<string, unknown>>("/species-stage-configs", { method: "POST", body });
...
export async function updateSpeciesStageConfig(id: string, payload: Partial<SpeciesStageConfigCreate>): Promise<SpeciesStageConfig | null> {
  ...
  const updated = await apiFetch<Record<string, unknown>>(`/species-stage-configs/${id}`, { method: "PUT", body });
...
export async function deleteSpeciesStageConfig(id: string): Promise<boolean> {
  await apiFetch(`/species-stage-configs/${id}`, { method: "DELETE" });
  return true;
}
