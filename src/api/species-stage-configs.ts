import type {
  SpeciesStageConfig,
  SpeciesStageConfigCreate,
} from "../types/species-stage-config";
...
function toUi(item: Record<string, unknown>): SpeciesStageConfig {
  return {
    ...
    growthStageName: item.growthStageName
      ? String(item.growthStageName)
      : undefined,
    ...
    amountPer100Fish:
      item.amountPer100Fish !== undefined
        ? Number(item.amountPer100Fish)
        : undefined,
    frequencyPerDay:
      item.frequencyPerDay !== undefined
        ? Number(item.frequencyPerDay)
        : undefined,
    maxStockingDensity:
      item.maxStockingDensity !== undefined
        ? Number(item.maxStockingDensity)
        : undefined,
    expectedDurationDays:
      item.expectedDurationDays !== undefined
        ? Number(item.expectedDurationDays)
        : undefined,
  };
}

export async function getSpeciesStageConfigs(
  searchTerm?: string,
): Promise<SpeciesStageConfig[]> {
  const path = searchTerm
    ? `/config/species-stage-configs?SearchTerm=${encodeURIComponent(searchTerm)}`
    : "/config/species-stage-configs";
...
export async function getSpeciesStageConfig(
  id: string,
): Promise<SpeciesStageConfig | null> {
  try {
    ...
    if (
      ...
      if (inner && typeof inner === "object")
        return toUi(inner as Record<string, unknown>);
    }
...
export async function createSpeciesStageConfig(
  payload: SpeciesStageConfigCreate,
): Promise<SpeciesStageConfig> {
  const body = { ...payload };
...
export async function updateSpeciesStageConfig(
  id: string,
  payload: Partial<SpeciesStageConfigCreate>,
): Promise<SpeciesStageConfig | null> {
  const body: Record<string, unknown> = {};
  Object.assign(body, payload);
  const updated = await apiFetch<Record<string, unknown>>(
    `/config/species-stage-configs/${id}`,
    { method: "PUT", body },
  );
