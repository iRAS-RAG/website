...
function toControlDevice(item: Record<string, unknown>): ControlDevice {
  return {
    ...
    controlDeviceTypeId: item.controlDeviceTypeId
      ? String(item.controlDeviceTypeId)
      : item.control_device_type_id
        ? String(item.control_device_type_id)
        : undefined,
    controlDeviceTypeName:
      (item.controlDeviceTypeName as string) ||
      (item.control_device_type_name as string) ||
      undefined,
    ...
    commandOn:
      (item.commandOn as string) || (item.command_on as string) || undefined,
    commandOff:
      (item.commandOff as string) || (item.command_off as string) || undefined,
  };
}

export async function getControlDevices(): Promise<ControlDevice[]> {
  const res = await apiFetch<unknown>("/hardwares/control-devices");
  const items = extractArray(res);
  return items.map((i) => toControlDevice(i as Record<string, unknown>));
}

export async function createControlDevice(
  payload: ControlDeviceUpsertPayload,
): Promise<ControlDevice> {
  const body: Record<string, unknown> = { ...payload };
  const created = await apiFetch<Record<string, unknown>>(
    "/hardwares/control-devices",
    { method: "POST", body },
  );
  return toControlDevice(created as Record<string, unknown>);
}

export async function updateControlDevice(
  id: string,
  payload: ControlDeviceUpsertPayload,
): Promise<ControlDevice | null> {
  const body: Record<string, unknown> = { ...payload };
  const updated = await apiFetch<Record<string, unknown>>(
    `/hardwares/control-devices/${id}`,
    { method: "PUT", body },
  );
...
export async function deleteControlDevice(id: string): Promise<boolean> {
  await apiFetch(`/hardwares/control-devices/${id}`, { method: "DELETE" });
  return true;
}
