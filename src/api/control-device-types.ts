...
export async function getControlDeviceTypes(): Promise<ControlDeviceType[]> {
  const res = await apiFetch<unknown>("/hardware/control-device-types");
  const items = extractArray(res);
  return items.map((i) => toUi(i as Record<string, unknown>));
}

export async function createControlDeviceType(payload: ControlDeviceTypeCreate): Promise<ControlDeviceType> {
  const body: Record<string, unknown> = { ...payload };
  const created = await apiFetch<Record<string, unknown>>("/hardware/control-device-types", { method: "POST", body });
...
export async function updateControlDeviceType(id: string, payload: Partial<ControlDeviceTypeCreate>): Promise<ControlDeviceType | null> {
  const body: Record<string, unknown> = {};
  Object.assign(body, payload);
  const updated = await apiFetch<Record<string, unknown>>(`/hardware/control-device-types/${id}`, { method: "PUT", body });
...
export async function deleteControlDeviceType(id: string): Promise<boolean> {
  await apiFetch(`/hardware/control-device-types/${id}`, { method: "DELETE" });
  return true;
}
